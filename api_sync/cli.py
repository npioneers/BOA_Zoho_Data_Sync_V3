...
def setup_verbose_logging() -> None:
    """Setup verbose logging configuration for detailed progress output."""
    # Create a specific logger for API sync operations only
    logger = logging.getLogger('api_sync_verbose')
    ...
    handler.setFormatter(logging.Formatter('[VERBOSE] %(message)s'))
    logger.addHandler(handler)
    
    # Prevent interference with other loggers
    logger.propagate = False
    
    # Also set up the main client logger for verbose output
    client_logger = logging.getLogger('api_sync.core.client')
...
def cmd_fetch(args) -> int:
    ...
    """
    try:
        # Setup verbose logging for detailed progress
        ...
        print(f"\n[FETCH] STARTING VERBOSE DATA FETCH")
