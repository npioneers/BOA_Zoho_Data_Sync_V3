...
class ApiLocalVerifier:
    ...
    def _verify_module(self, module: str, timestamp_dir: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get API count (count only, no data download)
            result["api_count"] = self.api_client.get_module_count(module)
            ...
            logger.info(f"Module {module}: Local={result['local_count']} (from {result['source_dir']}), API={result['api_count']}, Diff={result['difference']}")
